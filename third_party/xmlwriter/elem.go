...
func (e Elem) opened(n *node, w *Writer, prev NodeState) error {
	...
	if n.children == 0 && !e.Full && len(e.Content) == 0 {
		// Special modification done here to match byte for byte the mobile clients output for Kik XMPP.
		// In case Kik checks for this, we will be covered.
		if w.IosMode {
			// iOS always writes in long form when there are no children
			// <foo></foo>
			w.printer.WriteString("></")
			w.printer.WriteString(e.fullName())
			w.printer.WriteByte('>')
		} else {
			// Android uses KXmlSerializer, which writes a space before the self closing tag.
			// <foo />
			w.printer.WriteString(" />")
		}
	} else {
	...
	if len(e.Content) > 0 {
		...
		for _, c := range e.Content {
			...
			if err != nil {
				// TO DO: context about which index failed
				return err
			}
		}
	}
